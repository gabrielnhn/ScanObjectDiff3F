...
def run_rendering(device, mesh, mesh_vertices, num_views, H, W, add_angle_azi=0, add_angle_ele=0, use_normal_map=False):
    ...
    # Grid logic: steps * steps
    steps = int(math.sqrt(num_views))
    if steps < 1: steps = 1  # Safety for very low view counts
    ...
    # --- FIX START ---
    # Original code used 'num_views' here, which caused mismatch if num_views wasn't a perfect square.
    # We use 'rotation.shape[0]' which is exactly (steps * steps).
    actual_views = rotation.shape[0]
    batch_mesh = mesh.extend(actual_views)
    # --- FIX END ---
