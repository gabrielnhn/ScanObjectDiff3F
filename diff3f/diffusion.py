...
def rgb2normalmap(normal_map):
    ...
    # Case 1: Old Mesh Renderer output (H, W, 1, C)
    if normal_map.ndim == 4:
        normal_map = normal_map[:, :, 0, :3]
    # Case 2: Standard Image/Point Cloud output (H, W, C)
    elif normal_map.ndim == 3:
        normal_map = normal_map[:, :, :3]
        
    min_value = np.min(normal_map)
    max_value = np.max(normal_map)
    
    # Avoid division by zero if map is empty/flat
    if max_value - min_value > 1e-5:
        normalized_normal_map = np.where(normal_map != 0, (normal_map - min_value) / (max_value - min_value), 0)
    else:
        normalized_normal_map = normal_map
