...
from render2 import batch_render
...
def get_features_per_vertex(
    ...
    # --- MEMORY FIX START ---
    # We move vertices to CPU specifically for the cdist calculation.
    # Calculating this on GPU (even with sampling) requires massive VRAM that you don't have available.
    verts_cpu = mesh_vertices.cpu()
    
    if len(verts_cpu) > VERTEX_GPU_LIMIT:
        samples = random.sample(range(len(verts_cpu)), 10000)
        maximal_distance = torch.cdist(verts_cpu[samples], verts_cpu[samples]).max()
    else:
        maximal_distance = torch.cdist(verts_cpu, verts_cpu).max()
    
    del verts_cpu # Free CPU memory immediately
    # --- MEMORY FIX END ---
    ...
    for idx in tqdm(range(len(batched_renderings))):
        ...
        # Ensure world_coords is on device for ball_query
        world_coords = (
        ...
        # Ensure diffusion output is clean
        ...
        if bq:
            # Ball query runs on GPU using the mesh_vertices (already on GPU)
            queried_indices = (
                ball_query(
            ...
            # Fallback to cdist if bq is false
        ...
        # Memory Cleanup for the Loop
        del diffusion_output
        del aligned_features
        del aligned_dino_features
        del world_coords
        # Clearing cache every iteration is slow, but safe for 6GB VRAM
        # If it's too slow, move this to: if idx % 10 == 0:
        # torch.cuda.empty_cache() 
    ...
    if missing_features > 0:
        ...
        # Move these to CPU for the fill calculation to avoid VRAM spike at the end
        mv_missing = mesh_vertices[missing_indices].cpu()
        mv_filled = mesh_vertices[filled_indices].cpu()
        
        distances = torch.cdist(
            mv_missing, mv_filled, p=2
        )
        closest_vertex_indices = torch.argmin(distances, dim=1).cpu()
